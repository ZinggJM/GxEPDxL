...
#include "GxGDE060BAxL.h"

//#include "BitmapExamples.h"

GxGDE060BAxL::GxGDE060BAxL(GxDESTM32L_IO& io)
  : GxEPDxL(GxGDE060BAxL_WIDTH, GxGDE060BAxL_HEIGHT),
...
void GxGDE060BAxL::drawPixel(int16_t x, int16_t y, uint16_t color)
{
  ...
  switch (getRotation())
  {
    case 1:
      swap(x, y);
      x = GxGDE060BAxL_WIDTH - x - 1;
      break;
    case 2:
      x = GxGDE060BAxL_WIDTH - x - 1;
      y = GxGDE060BAxL_HEIGHT - y - 1;
      break;
    case 3:
      swap(x, y);
      y = GxGDE060BAxL_HEIGHT - y - 1;
  ...
  uint32_t i = x / 4 + y * GxGDE060BAxL_WIDTH / 4;
...
void GxGDE060BAxL::init(Stream* pDiagnosticOutput)
{
  IO.init(PB14);
  init_wave_table();
}

void GxGDE060BAxL::fillScreen(uint16_t color)
{
  uint8_t data;
  ...
  for (uint32_t x = 0; x < GxGDE060BAxL_BUFFER_SIZE; x++)
  {
    (*p_active_buffer)[x] = data;
  }
}

void GxGDE060BAxL::update()
{
...
void GxGDE060BAxL::drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color)
{
  drawBitmap(bitmap, x, y, w, h, color);
}

void  GxGDE060BAxL::drawBitmap(const uint8_t *bitmap, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color, int16_t mode)
{
  if (mode == bm_default) mode = bm_normal;
  drawBitmapBM(bitmap, x, y, w, h, color, mode);
}

void GxGDE060BAxL::drawPicture(const uint8_t *picture, uint32_t size)
{
  IO.powerOn();
  delay(25);
  for (uint16_t frame = 0; frame < GxGDE060BAxL_FRAME_END_SIZE - 2; frame++)
  {
    IO.start_scan();
    for (uint16_t line = 0; line < GxGDE060BAxL_HEIGHT; line++)
    {
      uint32_t x = line * GxGDE060BAxL_ROW_BUFFER_SIZE;
      uint16_t i = 0;
      for (; (i < GxGDE060BAxL_ROW_BUFFER_SIZE) && (x < size); i++, x++)
      {
        row_buffer[i] = wave_end_table[picture[x]][frame];
      }
      for (; (i < GxGDE060BAxL_ROW_BUFFER_SIZE); i++)
      {
        row_buffer[i] = wave_end_table[0xFF][frame];
      }
      IO.send_row(row_buffer, GxGDE060BAxL_ROW_BUFFER_SIZE, GxGDE060BAxL_CL_DELAY);
    }
    IO.send_row(row_buffer, GxGDE060BAxL_ROW_BUFFER_SIZE, GxGDE060BAxL_CL_DELAY);
  }
  delay(25);
...
void GxGDE060BAxL::drawBitmap(const uint8_t *bitmap, uint32_t size, int16_t m)
{
  IO.powerOn();
  delay(25);
  clear_display();
  for (uint16_t frame = 0; frame < GxGDE060BAxL_FRAME_END_SIZE; frame++)
  {
    IO.start_scan();
    for (uint16_t line = 0; line < GxGDE060BAxL_HEIGHT; line++)
    {
      uint32_t x = line * (GxGDE060BAxL_WIDTH / 8);
      uint16_t i = 0;
      for (; (i < GxGDE060BAxL_ROW_BUFFER_SIZE) && (x < size); i++)
      {
        if (0 == i % 2)
        {
...
void GxGDE060BAxL::erasePicture(const uint8_t *picture, uint32_t size)
{
  IO.powerOn();
  delay(25);
  for (uint16_t frame = 0; frame < GxGDE060BAxL_FRAME_BEGIN_SIZE; frame++)
  {
    IO.start_scan();
    for (uint16_t line = 0; line < GxGDE060BAxL_HEIGHT; line++)
    {
...
void GxGDE060BAxL::eraseBitmap(const uint8_t *bitmap, uint32_t size)
{
  IO.powerOn();
  delay(25);
  for (uint16_t frame = 0; frame < GxGDE060BAxL_FRAME_END_SIZE; frame++)
...
void GxGDE060BAxL::eraseDisplay()
{
  IO.powerOn();
  delay(25);
  clear_display();
...
void GxGDE060BAxL::init_wave_table(void)
{
  int frame, num;
  unsigned char tmp, value;

  for (frame = 0; frame < GxGDE060BAxL_FRAME_BEGIN_SIZE; frame++)
  {
  ...
  //wave_end_table
  for (frame = 0; frame < GxGDE060BAxL_FRAME_END_SIZE; frame++)
  {
    for (num = 0; num < WAVE_TABLE_SIZE; num++)
    {
      tmp = 0;
...
void GxGDE060BAxL::clear_display()
{
  for (uint16_t frame = 0; frame < GxGDE060BAxL_FRAME_BEGIN_SIZE; frame++)
  {
    IO.start_scan();
    for (uint16_t line = 0; line < GxGDE060BAxL_HEIGHT; line++)
    {
      for (uint16_t i = 0; (i < GxGDE060BAxL_ROW_BUFFER_SIZE); i++)
      {
...
#if 0
void GxGDE060BAxL::DisplayTestPicture(uint8_t nr)
{
  unsigned char *ptr;
  ...
  for (uint16_t frame = 0; frame < GxGDE060BAxL_FRAME_BEGIN_SIZE; frame++)
  {
    ...
    {
      uint32_t x = line * GxGDE060BAxL_ROW_BUFFER_SIZE;
      for (uint16_t i = 0; i < GxGDE060BAxL_ROW_BUFFER_SIZE; i++, x++)
      {
...
void GxGDE060BAxL::fillScreenTest()
{
  for (uint32_t x = 0; x < GxGDE060BAxL_BUFFER_SIZE; x++)
  {
    if (x < GxGDE060BAxL_BUFFER_SIZE * 3 / 8) (*p_active_buffer)[x] = 0x00;
    else if (x < GxGDE060BAxL_BUFFER_SIZE / 2) (*p_active_buffer)[x] = 0x55;
    else if (x < GxGDE060BAxL_BUFFER_SIZE * 3 / 4) (*p_active_buffer)[x] = 0xCC;
