...
GxDESTM32L_IO::GxDESTM32L_IO()
{
  _pwr_led = PB12;
}

void GxDESTM32L_IO::init(uint8_t power_on_led)
{
...
void GxDESTM32L_IO::powerOn(void)
{
  digitalWrite(_pwr_led, LOW);
...
void GxDESTM32L_IO::powerOff(void)
{
...
void GxDESTM32L_IO::start_scan(void)
{
  EPD_XSPV_H;
...
void GxDESTM32L_IO::send_row(uint8_t row_data[], uint16_t row_size, uint32_t delay_time)
{
  EPD_LE_H;
  EPD_CL_L;
  EPD_CL_H;
...
void GxDESTM32L_IO::delay35ns(uint32_t nCount)
{
  for (; nCount != 0; nCount--)
  {
    t_delay += nCount;
