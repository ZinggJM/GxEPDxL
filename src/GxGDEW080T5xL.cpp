// Class GxGDEW080T5xL : display class for GDE060BA e-paper display from Dalian Good Display Inc.
//
// Created by Jean-Marc Zingg based on demo code from Good Display for red DESTM32-L board.
//
// The GxGDEW080T5xL class is not board specific, but relies on FMSC SRAM for buffer memory, as available on DESTM32-L,
...
#include "GxGDEW080T5xL.h"

//#include "BitmapExamples.h"

GxGDEW080T5xL::GxGDEW080T5xL(GxDESTM32L_IO& io)
  : GxEPDxL(GxGDEW080T5xL_WIDTH, GxGDEW080T5xL_HEIGHT),
...
void GxGDEW080T5xL::drawPixel(int16_t x, int16_t y, uint16_t color)
{
  ...
  switch (getRotation())
  {
    case 1:
      swap(x, y);
      x = GxGDEW080T5xL_WIDTH - x - 1;
      break;
    case 2:
      x = GxGDEW080T5xL_WIDTH - x - 1;
      y = GxGDEW080T5xL_HEIGHT - y - 1;
      break;
    case 3:
      swap(x, y);
      y = GxGDEW080T5xL_HEIGHT - y - 1;
  ...
  uint32_t i = x / 4 + y * GxGDEW080T5xL_WIDTH / 4;
...
void GxGDEW080T5xL::init(Stream* pDiagnosticOutput)
{
  IO.init(PB15);
  init_wave_table();
}

void GxGDEW080T5xL::fillScreen(uint16_t color)
{
  uint8_t data;
  ...
  for (uint32_t x = 0; x < GxGDEW080T5xL_BUFFER_SIZE; x++)
  {
    (*p_active_buffer)[x] = data;
  }
}

void GxGDEW080T5xL::update()
{
...
void GxGDEW080T5xL::drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color)
{
  drawBitmap(bitmap, x, y, w, h, color);
}

void  GxGDEW080T5xL::drawBitmap(const uint8_t *bitmap, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color, int16_t mode)
{
  if (mode == bm_default) mode = bm_normal;
  drawBitmapBM(bitmap, x, y, w, h, color, mode);
}

void GxGDEW080T5xL::drawPicture(const uint8_t *bitmap, uint32_t size)
{
  IO.powerOn();
  delay(25);
  for (uint16_t frame = 0; frame < GxGDEW080T5xL_FRAME_END_SIZE - 2; frame++)
  {
    IO.start_scan();
    for (uint16_t line = 0; line < GxGDEW080T5xL_HEIGHT; line++)
    {
      uint32_t x = line * GxGDEW080T5xL_ROW_BUFFER_SIZE;
      uint16_t i = 0;
      for (; (i < GxGDEW080T5xL_ROW_BUFFER_SIZE) && (x < size); i++, x++)
      {
        row_buffer[i] = wave_end_table[bitmap[x]][frame];
      }
      for (; (i < GxGDEW080T5xL_ROW_BUFFER_SIZE); i++)
      {
        row_buffer[i] = wave_end_table[0xFF][frame];
      }
      IO.send_row(row_buffer, GxGDEW080T5xL_ROW_BUFFER_SIZE, GxGDEW080T5xL_CL_DELAY);
    }
    IO.send_row(row_buffer, GxGDEW080T5xL_ROW_BUFFER_SIZE, GxGDEW080T5xL_CL_DELAY);
  }
  delay(25);
...
void GxGDEW080T5xL::drawBitmap(const uint8_t *bitmap, uint32_t size, int16_t m)
{
  IO.powerOn();
  delay(25);
  clear_display();
  for (uint16_t frame = 0; frame < GxGDEW080T5xL_FRAME_END_SIZE; frame++)
  {
    IO.start_scan();
    for (uint16_t line = 0; line < GxGDEW080T5xL_HEIGHT; line++)
    {
      uint32_t x = line * (GxGDEW080T5xL_WIDTH / 8);
      uint16_t i = 0;
      for (; (i < GxGDEW080T5xL_ROW_BUFFER_SIZE) && (x < size); i++)
      {
        if (0 == i % 2)
        {
...
void GxGDEW080T5xL::erasePicture(const uint8_t *bitmap, uint32_t size)
{
  IO.powerOn();
  delay(25);
  for (uint16_t frame = 0; frame < GxGDEW080T5xL_FRAME_BEGIN_SIZE; frame++)
  {
    IO.start_scan();
    for (uint16_t line = 0; line < GxGDEW080T5xL_HEIGHT; line++)
    {
...
void GxGDEW080T5xL::eraseBitmap(const uint8_t *bitmap, uint32_t size)
{
  IO.powerOn();
  delay(25);
  for (uint16_t frame = 0; frame < GxGDEW080T5xL_FRAME_END_SIZE; frame++)
...
void GxGDEW080T5xL::eraseDisplay() 
{
  IO.powerOn();
  delay(25);
  clear_display();
...
void GxGDEW080T5xL::init_wave_table(void)
{
  int frame, num;
  unsigned char tmp, value;

  for (frame = 0; frame < GxGDEW080T5xL_FRAME_BEGIN_SIZE; frame++)
  {
  ...
  //wave_end_table
  for (frame = 0; frame < GxGDEW080T5xL_FRAME_END_SIZE; frame++)
  {
    for (num = 0; num < WAVE_TABLE_SIZE; num++)
    {
      tmp = 0;
...
void GxGDEW080T5xL::clear_display()
{
  for (uint16_t frame = 0; frame < GxGDEW080T5xL_FRAME_BEGIN_SIZE; frame++)
  {
    IO.start_scan();
    for (uint16_t line = 0; line < GxGDEW080T5xL_HEIGHT; line++)
    {
      for (uint16_t i = 0; (i < GxGDEW080T5xL_ROW_BUFFER_SIZE); i++)
      {
...
#if 0
void GxGDEW080T5xL::DisplayTestPicture(uint8_t nr)
{
  unsigned char *ptr;
  ...
  for (uint16_t frame = 0; frame < GxGDEW080T5xL_FRAME_BEGIN_SIZE; frame++)
  {
    ...
    {
      uint32_t x = line * GxGDEW080T5xL_ROW_BUFFER_SIZE;
      for (uint16_t i = 0; i < GxGDEW080T5xL_ROW_BUFFER_SIZE; i++, x++)
      {
...
void GxGDEW080T5xL::fillScreenTest()
{
  for (uint32_t x = 0; x < GxGDEW080T5xL_BUFFER_SIZE; x++)
  {
    if (x < GxGDEW080T5xL_BUFFER_SIZE * 3 / 8) (*p_active_buffer)[x] = 0x00;
    else if (x < GxGDEW080T5xL_BUFFER_SIZE / 2) (*p_active_buffer)[x] = 0x55;
    else if (x < GxGDEW080T5xL_BUFFER_SIZE * 3 / 4) (*p_active_buffer)[x] = 0xCC;
